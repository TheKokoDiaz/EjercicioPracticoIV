...
('codigo', 'class CuentaBancaria: def __init__(self, titular, saldo=0): self.titular = titular; self.saldo = saldo; self.movimientos = []; def depositar(self, monto): if monto <= 0: raise ValueError("El monto debe ser positivo"); self.saldo += monto; self.movimientos.append(("deposito", monto)); def retirar(self, monto): if monto > self.saldo: raise ValueError("Fondos insuficientes"); self.saldo -= monto; self.movimientos.append(("retiro", monto)); def historial(self): for tipo, monto in self.movimientos: print(f"{tipo}: {monto}"); cuenta = CuentaBancaria("Laura", 500); cuenta.depositar(150); cuenta.retirar(200); cuenta.historial(); print(f"Saldo final: {cuenta.saldo}")'),
('codigo', 'def quicksort(lista): if len(lista) <= 1: return lista; pivote = lista[len(lista) // 2]; menores = [x for x in lista if x < pivote]; iguales = [x for x in lista if x == pivote]; mayores = [x for x in lista if x > pivote]; return quicksort(menores) + iguales + quicksort(mayores); def busqueda_binaria(lista, objetivo): inicio, fin = 0, len(lista) - 1; while inicio <= fin: medio = (inicio + fin) // 2; if lista[medio] == objetivo: return medio; elif lista[medio] < objetivo: inicio = medio + 1; else: fin = medio - 1; return -1; numeros = [9, 3, 7, 1, 8, 2, 5, 4, 6]; ordenados = quicksort(numeros); print(ordenados); print(busqueda_binaria(ordenados, 7))'),
('codigo', 'import csv; from dataclasses import dataclass; @dataclass; class Producto: nombre: str; precio: float; cantidad: int; def total(self): return self.precio * self.cantidad; def leer_inventario(ruta): productos = []; with open(ruta, newline="", encoding="utf-8") as archivo: lector = csv.reader(archivo); next(lector); for fila in lector: nombre, precio, cantidad = fila; productos.append(Producto(nombre, float(precio), int(cantidad))); return productos; def valor_total(productos): return sum(producto.total() for producto in productos); productos = leer_inventario("inventario.csv"); for producto in productos: print(f"{producto.nombre}: {producto.total():.2f}"); print(f"Valor total del inventario: {valor_total(productos):.2f}")'),
('codigo', 'def generador_fibonacci(limite): a, b = 0, 1; while a < limite: yield a; a, b = b, a + b; def es_primo(numero): if numero < 2: return False; for divisor in range(2, int(numero ** 0.5) + 1): if numero % divisor == 0: return False; return True; def primos_hasta(limite): return [n for n in range(2, limite) if es_primo(n)]; def registrar_tiempo(funcion): def envoltura(*args, **kwargs): import time; inicio = time.time(); resultado = funcion(*args, **kwargs); duracion = time.time() - inicio; print(f"{funcion.__name__} tardo {duracion:.4f} segundos"); return resultado; return envoltura; @registrar_tiempo; def calcular_todo(): fibonacci = list(generador_fibonacci(100)); primos = primos_hasta(100); return fibonacci, primos; fibonacci, primos = calcular_todo(); print(fibonacci); print(primos)'),
('codigo', 'from collections import defaultdict; class GestorTareas: def __init__(self): self.tareas = defaultdict(list); def agregar(self, categoria, descripcion, prioridad="media"): self.tareas[categoria].append({"descripcion": descripcion, "prioridad": prioridad, "completada": False}); def completar(self, categoria, indice): try: self.tareas[categoria][indice]["completada"] = True; except IndexError: print("Esa tarea no existe"); def pendientes(self, categoria): return [t for t in self.tareas[categoria] if not t["completada"]]; def resumen(self): for categoria, lista in self.tareas.items(): completadas = sum(1 for t in lista if t["completada"]); print(f"{categoria}: {completadas}/{len(lista)} completadas"); gestor = GestorTareas(); gestor.agregar("trabajo", "Enviar reporte semanal", "alta"); gestor.agregar("trabajo", "Revisar correos", "baja"); gestor.agregar("personal", "Comprar viveres"); gestor.completar("trabajo", 0); gestor.resumen()');
...
ALTER TABLE Usuario ADD COLUMN intentos_fallidos INT NOT NULL DEFAULT 0, ADD COLUMN bloqueado_hasta DATETIME NULL;